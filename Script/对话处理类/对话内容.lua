...
-- @Last Modified time: 2024-01-28 06:19:56
--======================================================================--
local 对话处理类 = class()
...
function 对话处理类:索取对话内容(id,数字id,序号,内容)
	...
	elseif self.地图编号<100000 and 取两点距离(self.比较xy,self.假人数据)>=500 then
	    ...
	    elseif self.临时数据[2]=="节日活动" then
	        if 玩家数据[数字id].角色:取任务(411)~=0 then
	         	if 任务数据[玩家数据[数字id].角色:取任务(411)].分类==3 or 任务数据[玩家数据[数字id].角色:取任务(411)].分类==4 then
	            	self.临时数据[4][#self.临时数据[4]+1]="上交物品"
	            	玩家数据[数字id].最后操作="节日药品"
	        	end
	      	end
...
---嘎嘎任务补充
      if self.临时数据[2]=="颜如玉" then
	        if 玩家数据[数字id].角色:取任务(19941)~=0 then
	            self.临时数据[4][#self.临时数据[4]+1]="文韵墨香答题"
	        end
	    end
	    if self.临时数据[2]=="李世民" then
	        if 玩家数据[数字id].角色:取任务(19943)~=0 then
	            self.临时数据[4][#self.临时数据[4]+1]="文韵墨香寻找李世民"
	        end
	    end
	    if self.临时数据[2]=="陈员外" then
	        if 玩家数据[数字id].角色:取任务(19944)~=0 then
	            self.临时数据[4][#self.临时数据[4]+1]="文韵墨香帮陈员外砍木头"
	        end
	    end
	    if self.临时数据[2]=="长安西域使者" then
	        if 玩家数据[数字id].角色:取任务(19946)~=0 then
	            self.临时数据[4][#self.临时数据[4]+1]="我寻找到了西域使者"
	        end
	    end
	    if self.临时数据[2]=="长寿郊外西域使者" then
	        if 玩家数据[数字id].角色:取任务(19946)~=0 then
	            self.临时数据[4][#self.临时数据[4]+1]="我寻找到了西域使者"
	        end
	    end
	    if self.临时数据[2]=="大唐国境西域使者" then
	        if 玩家数据[数字id].角色:取任务(19946)~=0 then
	            self.临时数据[4][#self.临时数据[4]+1]="我寻找到了西域使者"
	        end
	    end
	    if 文韵墨香~=nil and 文韵墨香[数字id]~=nil and 文韵墨香[数字id].送信~=nil and 文韵墨香[数字id].送信.送信人~=false then
	       if self.临时数据[2]==文韵墨香[数字id].送信.送信人 then
	            if 玩家数据[数字id].角色:取任务(19947)~=0 then
	               self.临时数据[4][#self.临时数据[4]+1]="完成文韵送信"
	            end
	       end
	    end
		...
		if 玩家数据[数字id].角色:取任务(353) ~= 0 then
		    ...
		    if self.临时数据[2]==任务数据[任务id].人物 and self.地图编号==任务数据[任务id].人物地图 then
		        ...
		        --if 副本数据.水陆大会.进行[副本id].装潢>=10 and 副本数据.水陆大会.进行[副本id].邀请>=10 then
		        	if 副本数据.水陆大会.进行[副本id].邀请>=1 then
      ...
      if 玩家数据[数字id].队长 then
        if self.临时数据[2]=="马副将" or self.临时数据[2]=="御林军" or self.临时数据[2]=="赵捕头" or self.临时数据[2]=="酒店老板" or self.临时数据[2]=="袁天罡" then
          if 玩家数据[数字id].队伍 == nil or 队伍数据[玩家数据[数字id].队伍] == nil then --or 队伍数据[玩家数据[数字id].队伍].成员数据
            return
          end
...
function 对话处理类:取对话内容(ID,编号,页数,已经在任务中,数字id)
	...
	if ID == 1003 then--桃园
		if 编号 == 20 then
			...
			return{"巫蛮儿","新手接待师",wb[取随机数(1,#wb)],xx}
		--end
		-- wb[1] = "欢迎来到#P梦幻情怀#W这是你没有玩过的#S全新版本\n仿官方设置经典耐玩,#Y不伤肝\n全网#R战斗细节#W最好,#G打击感#W超棒\n独家补全了所有#G音效,#Y特效,#S造型\n全官方#Z剧情流程#W好玩到爆炸\n更多游戏内容,还请您深入体验"
		-- 	xx = {"剧情任务补领"}
		...
		elseif 编号 == 21 then
			wb[1] = "我是#S七游#的#R最大BOSS#G别人都叫我#P狼大宝#Y哇呜~#\n#Y沿着河边的路往下走，就可以看到霞姑娘了"
			-- xx = {"飞燕女","英女侠","巫蛮儿","逍遥生","剑侠客","狐美人","骨精灵","杀破狼"
			-- 	,"巨魔王","虎头怪","舞天姬","玄彩娥","羽灵神","神天兵","龙太子","桃夭夭"
			-- 	,"偃无师","鬼潇潇"}
			return{"神天兵","狼宝宝",wb[取随机数(1,#wb)],xx}
		end
	elseif ID == 1501 then--建邺城
		...
		elseif 编号 == 3 then
			...
			return{"男人_苦力","吹牛王",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 4 then
			return{"剑侠客","刻晴","由管理员赠送的银子和储备也可以在我这里领取哟",
			{"领取推广奖励","领取银子","领取储备"}}
		elseif 编号 == 5 then
	  ...
	  if 成就数据[数字id].梦战一章==1 and (成就数据[数字id].梦战二章==0 or 成就数据[数字id].梦战二章==nil) then
		...
		if 玩家数据[数字id].角色:取任务(502) ~= 0 and (任务数据[玩家数据[数字id].角色:取任务(502)].进程 == 3 or 任务数据[玩家数据[数字id].角色:取任务(502)].进程 == 4
			or 任务数据[玩家数据[数字id].角色:取任务(502)].进程 == 5 or 任务数据[玩家数据[数字id].角色:取任务(502)].进程 == 6 or 任务数据[玩家数据[数字id].角色:取任务(502)].进程 == 7) then
    ...
    elseif ID == 6043 then
		...
		wb[1] = "你看我美吗?"
		xx={"触发剧情","暂时不用了"}
		return {"普陀_接引仙女","宠物仙子",wb[取随机数(1,#wb)],xx}
		end
		...
		xx={"询问一番","暂时不用了"}
		return {"男人_镖头","张捕头",wb[取随机数(1,#wb)],xx}
		end
		...
		wb[1] = "哈哈,被你发现了!"
		xx={"少说废话,看打","暂时不用了"}
		return {"强盗","江南小盗",wb[取随机数(1,#wb)],xx}
		end
		wb[1] = "哈哈,被你发现了!"
		return {"强盗","江南小盗",wb[取随机数(1,#wb)],xx}
		elseif 编号==5 then
		...
		wb[1] = "花姑凉,大大的!"
		xx={"放开那个女孩,让我来!","暂时不用了"}
		return {"巨魔王","黄狮王",wb[取随机数(1,#wb)],xx}
		end
		...
		wb[1] = "我一定会报仇的!"
		xx = {"偷听它们说话","暂时不用了"}
		return{"大海龟","大大龟",wb[取随机数(1,#wb)],xx}
		end
		...
		elseif 编号 == 2 then
		wb[1] = "跟着龟哥混肯定没错!"
		return{"海毛虫","果毛虫",wb[取随机数(1,#wb)],xx}
	    end
	    elseif ID == 6045 then
		if 编号 == 1 then
		...
		xx = {"毛贼,快把百仙丹交出来","暂时不用了"}
		return{"强盗","狂盗",wb[取随机数(1,#wb)],xx}
		end
		wb[1] = "我逃跑技术可是登峰造极,我跑!"
		return{"强盗","狂盗",wb[取随机数(1,#wb)],xx}
	    end
  ...
  elseif ID == 1001 then
		...
		elseif 编号 == 12 then
			wb[1] = "少侠请选择要为你传送的门派"
			--xx = {"方寸山","女儿村","神木林","化生寺","大唐官府","盘丝洞","阴曹地府","无底洞","魔王寨","狮驼岭","天宫","普陀山","凌波城","五庄观","女魃墓","龙宫"}
    	...
    	elseif 编号 == 16 then
			...
			xx={"学习调息","学习变化之术","学习奇门遁甲","学习妙手空空","学习打坐","学习宝石工艺","学习仙灵店铺","学习坐骑飞行","购买五色旗盒","兑换进阶变身卡(50W银子一次)"}
    	...
    	elseif 编号 == 21 then
			...
			xx={"勇闯通天塔","人物洗点","法术认证","取消法术认证","角色转换","装备转换","武器染色","补充法宝灵气","藏宝阁物品和银子取回"}
        ...
        elseif 编号 == 39 then
			wb[1] = "少侠想做什么呢？#Y(种族任务暂未开启)"
			return{"男人_衙役","王捕头",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 40 then
			wb[1] = "我这里可以帮助你取消一些效果和修复#G卡装备不能穿戴#的问题"
            --xx={"取消人物变身卡","取消助战变身卡","取消罗羹效果","修复卡装备","修复法宝"}
           xx={"取消人物变身卡","取消助战变身卡","取消罗羹效果"}
			return{"男人_老书生","效果取消员",wb[取随机数(1,#wb)],xx}
	  	elseif 编号 == 41 then
		...
		elseif 编号 == 52 then
	        wb[1] = "哎哟不错,很吊哦!!,我会根据你的属性自动调整我的属性哦！"
      		xx = {"伤害测试","我路过瞧瞧而已"}
       		return {"周杰伦","周吊伦",wb[取随机数(1,#wb)],xx}
		elseif 编号 >= 53 and 编号<= 59 then
			wb[1] = "皇宫境内，禁止喧哗。"
			return{"男人_马副将","御林军",wb[取随机数(1,#wb)],xx}
		...
		elseif 编号 == 63 then
			...
			if 池定制 then
		      xx = {"剑会天下奖励领取","乌鸡副本奖励领取","车迟副本奖励领取","水陆副本奖励领取","通天副本奖励领取"}
			else
			xx = {"剑会天下奖励领取","乌鸡副本奖励领取","车迟副本奖励领取","水陆副本奖励领取","通天副本奖励领取","添加装备无级别(500W经验和2000W银子一次20%几率)","添加灵饰无级别(500W经验和2000W银子一次30%几率)"}
			end
			return{"偃无师","奖励发放员",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 64 then
			wb[1] = "我是负责#R天下英雄大会#W的主持人，你找我有什么事呢？"
			xx={"我要参加天下英雄大会","我要打听天下英雄会消息","我要领取比赛奖励","我要用英雄币购买物品","进入比赛会场","我要查询天下英雄榜及我的积分","我没什么事"}
			xx={"进入比赛会场","我要领取比赛奖励","我要打听天下英雄会消息"}
    	...
    	elseif 编号 == 66 then
    		xx = {}
    		if 神树抽奖 then
    		    wb[1] = "在我这里可以进行抽奖活动，少侠需要参与吗？\n#Y请保持背包空格有1个以上，否则将损失奖品"
				xx = {"我要进行抽奖(300仙玉一次)","过来看看热闹"}
			elseif 吊游定制 then
    	...
    	elseif 编号 == 69 then
			...
			return{"男人_兰虎","家园车夫",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 70  then
		...
		elseif 编号 == 79 then
			...
			return{"鬼潇潇","女猪脚",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 80 then
		...
		elseif 编号 == 81 then
			wb[1] = "妖魔积分抽奖，祝君好运！"
            xx={"妖魔积分抽奖(300点一次,保持包裹格数充足)","抽奖(200仙玉一次,保持包裹格数充足)" ,"进入武神坛地图" }
            --xx={"300积分一次(保持包裹格数充足)","进入武神坛地图" }
			return{"剑侠客","武神坛使者",wb[取随机数(1,#wb)],xx}

		elseif 编号 == 82 then ---嘎嘎 跨服
			-- wb[1] = "老子是赌大仙,来上一把,骚年！"
			-- xx={"严禁赌博"}
			-- return{"男人_老书生","赌日天",wb[取随机数(1,#wb)],xx}
      wb[1] = "此处可以连通所有区服,里面有各种各样的活动怪物哦！奖励超级丰富"
			xx = {"我要跨服","待定123","路过"}
			return {"男人_太上老君","神秘老人",wb[1],xx}
	    ...
	    elseif 编号 == 83 then
			...
			return{"游奕灵官","天降辰星",wb[取随机数(1,#wb)],xx}

		elseif 编号 == 84 then
        	 wb[1]=string.format("/瑞雪兆丰年,祝愿各位玩家新的一年 生意红红火火 事业节节攀升 家庭幸福美满 身体健康长寿。当前雪人进度:鼻子(%s/%s)、帽子(%s/%s)、眼睛(%s/%s),任务完成后全服在线玩家均可获得新年奖励哟！",雪人活动.雪人鼻子,雪人活动.上限,雪人活动.雪人帽子,雪人活动.上限,雪人活动.雪人眼睛,雪人活动.上限)
        	 xx={"看看有什么可以帮忙","我来堆雪人","查看雪人积分","雪人积分抽奖"}
        	--wb[1] = "雪人活动暂时先不开放了"
        	...
        	if 雪人积分开关 then
      		...
      		return{"雪人5","雪人",wb[取随机数(1,#wb)],xx}
        elseif 编号 == 85 then
			wb[1] = "情怀至今已经#Y三个年头#,#G感谢大家的支持#2\n这次给大家带来的是#S全新剧情版#3\n融合了#Y官方正常西游剧情线#+#G梦战旭梦魔心剧情线#+#P梦中梦突破剧情线#4\n#S让你们剧情玩法爽个够!#25"
		    return{"偃无师","刻晴",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 86 then
			wb[1] = "梦幻西游,#S是兄弟就来砍我!#114"
		    return{"龙太子","村花",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 87 then
			wb[1] = "大家游戏愉快#77店小二在呼唤我#81\n#Y隐藏型大佬,#R情怀2里面#多个剧情#G难度BOSS首杀玩家#2"
		    return{"剑侠客","全功仔",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 88 then
			wb[1] = "都是#Y群里兄弟#看得起，我tm一念之差#G当了个老大#1"
		    return{"虎头怪","哒总",wb[取随机数(1,#wb)],xx}
		 elseif 编号 == 89 then
			wb[1] = "想要挑战#S夏夏夏,啊胡,樱岛麻衣,#败给了#G鸳鸯以及菊花嘛#3"
		    return{"巨魔王","菊花",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 90 then
			wb[1] = "#S情怀真好玩#,俺要#P肝三天三夜"
		    return{"玄彩娥","曲终人散",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 91 then
			wb[1] = "#R哒总#天下第一！#2"
		    return{"剑侠客","季珪",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 92 then
			wb[1] = "梦幻西游真好玩#5"
		    return{"剑侠客","冷少",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 93 then
			wb[1] = "有刻晴的才是#Y真情怀#7"
		    return{"剑侠客","风清柒、",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 94 then
			wb[1] = "要不要借你掉装备（#Y可以单帐号借一把为一天的90武器#）#1"
		    return{"剑侠客","阿饼",wb[取随机数(1,#wb)],xx}
		 elseif 编号 == 95 then
			wb[1] = "我们#S习武之人#可能没什么用，但我们能#P保护有用的人#16"
		    return{"剑侠客","江湖小虾米",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 96 then
			wb[1] = "就是玩命肝#15"
		    return{"龙太子","踏月夜归人",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 97 then
			wb[1] = "#S大家一起愉快玩耍!#88"
		    return{"龙太子","猫爸",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 98 then
			wb[1] = "我变秃了，也变强了#91"
		    return{"逍遥生","书生",wb[取随机数(1,#wb)],xx}
		 elseif 编号 == 99 then
			wb[1] = "#G情怀一入深似海，#Y从此双肝是路人#29"
		    return{"逍遥生","扳手",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 100 then
			wb[1] = "随便打几个就行了#109"
		    return{"剑侠客","神器制造者",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 101 then
			wb[1] = "#S我这里没有彩蛋,走开吧#99"
		    return{"逍遥生","六一大侠",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 102 then
			wb[1] = "情怀我罩着的,如果掉线了,就是我的搞的#3"
		    return{"狐美人","大鱼海棠",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 103 then
			wb[1] = "情怀天下第一#28"
		    return{"剑侠客","方寸是星河",wb[取随机数(1,#wb)],xx}
		elseif 编号 == 104 then
			wb[1] = "少侠集齐99个神兜兜兑换超级神兽,也可消耗99个神兜兜兑换千亿兽决"
			xx = {"我要兑换超级神兽","我要兑换超级神兽.神兜兜(专属)","我要兑换千亿兽决","我先告辞"}
			return{"超级神兔","神兽使者",wb[取随机数(1,#wb)],xx}
    ...
    elseif 编号 == 105 then
		...
		elseif 编号 == 106 then
			wb[1] = "正是举国科举期间,今年考生众多,一时忙不过来,想请少侠帮助一二,少侠是否愿意?"
			xx = {"领取任务","任务进度","取消任务","路过"}
			return {"考官2","礼部侍郎官",wb[1],xx}
		elseif 编号 == 107 then
			wb[1] = "我是文韵墨香长安的西域使者"
			return {"男人_马副将","长安西域使者",wb[1],xx}
	...
	elseif ID == 1022 then
		...
		elseif 编号 == 2 then
			...
			xx = {"普通打造","强化打造","元身打造","关于打造方式的介绍","路过"}
  	...
  	elseif ID == 1020 then
		...
		elseif 编号 == 2 then
			...
			wb[1] = "你好啊!"
			xx = {"触发剧情","再见"}
			return{"男人_武器店老板","武器店老板",wb[sj(1,#wb)],xx}
			end
			wb[1] = "客官想要什么兵器？"
			wb[2] = "行走江湖不能两手空空，来挑一件趁手的兵器吧。"
			wb[3] = "这里的风景还不错吧。"
	...
	elseif ID == 1054 then
		if 编号 == 1 then
			xx = {"交谈","给予","师门任务","学习技能","兑换乾元丹","土豪兑换乾元丹","退出门派","路过，回来看看师傅！"}
	...
	elseif ID == 1204 then
		if 编号 == 1 then
		    wb[1] = "#S生死劫1层奖励#：#G8-12级星辉石#\n#S生死劫2层奖励#：#G8-12级各种宝石\n#S生死劫3层奖励#：#G5亿银子和130-140级书铁\n#S生死劫4层奖励#：#G固定140级书铁\n#S生死劫5层奖励#：#G固定140级灵饰书\n#S生死劫6层奖励#：#G固定140级灵饰铁\n#S生死劫7层奖励#：#G全套130级武神坛无级别礼包\n#S生死劫8层奖励#：#G全套140级武神坛无级别礼包\n#S生死劫9层奖励#：#G固定150级书铁"
        	xx = {"我已经准备好了，请开始挑战吧"}
   ...
   elseif ID == 1117 then
	...
	elseif ID == 1141 then
 	...
 	elseif ID == 1111 then
		...
		if 编号 == 1 then
	    wb[1] = "111"
		xx = {"接桃园任务","接建邺任务","接玄奘身世","接取梦战第一章","接取梦战第二章","接取新手任务","取消桃园任务","取消建邺任务","取消玄奘身世","取消梦战第一章","取消梦战第二章","取消新手指引"}
		return{"龙太子","秋风的回忆",wb[取随机数(1,#wb)],xx}
		end

		elseif ID== 6040 then --梦战天宫
		if 编号 == 1 then
		...
		wb[1] = "大胆狂徒,竟敢擅闯兜率宫"
		xx = {"切磋切磋","取消"}
		return{"灵鹤","三清",wb[取随机数(1,#wb)],xx}
		end
		wb[1] = "滚开~"
		return{"灵鹤","三清",wb[取随机数(1,#wb)],xx}
		end
		elseif ID== 6042 then --梦战凌霄宝殿
		if 编号 == 1 then
	...
	elseif ID == 1114 then
		if 编号 == 1 then
			wb[1] = "目前来说以你的修为已经到头了，如果想百尺竿头，更上一步的话，除非能入于化境！"
			 -- if 玩家数据[数字id].账号== "g4215567" then
			--xx = {"什么是化境？","我想入于化境，请上仙指点一二","入于化境后有什么变化？","取消飞升任务","我已经通过考验，请帮我入于化境","渡劫飞升","直接战斗渡劫","直接战斗飞升"}
			-- else
		    --xx = {"什么是飞升化境？","我想领取飞升化境任务","我已经通过考验，请帮我入于飞升化境","取消飞升任务"}
			xx = {"我想领取飞升化境任务","我已经通过考验，请帮我入于飞升化境","取消飞升任务"}
			-- end
	...
	elseif ID == 1092 then
    	...
    	elseif 编号 == 19 then
			...
			wb[1] = "敢闯我们的地盘，胆子不小啊。"
			xx = {"开启战斗"}
			return{"古代瑞兽","蓝火兽",wb[取随机数(1,#wb)],xx}
			end
      		wb[1] = "敢闯我们的地盘，胆子不小啊。"
      		return{"古代瑞兽","蓝火兽",wb[取随机数(1,#wb)],xx}
      	elseif 编号 == 20 then
			...
			wb[1] = "哼，你们哪里跑！"
			xx = {"开启战斗"}
			return{"大蝙蝠","吸血蝙蝠",wb[取随机数(1,#wb)],xx}
			end
      		wb[1] = "哼，你们哪里跑！"
      		return{"大蝙蝠","吸血蝙蝠",wb[取随机数(1,#wb)],xx}
      	elseif 编号 == 21 then
  ...
  elseif ID == 1174 then
 	...
 	elseif ID == 1091 then
		...
		elseif 编号 == 5 then
			...
			return {"赌徒","路人甲",wb[1],xx}
	  elseif 编号 == 6 then
			wb[1] = "我是文韵墨香长寿郊外的西域使者"
			return {"男人_马副将","长寿郊外西域使者",wb[1],xx}
		end
	elseif ID == 1168 then
		if 编号 == 1 then
  	...
  	elseif ID == 1110 then
    	...
    	elseif 编号==11 then
      	...
      	elseif 编号==18 then
    		...
    		wb[1] = "看什么看,没见过帅哥吗?"
    		xx = {"触发剧情"}
    		return {"赌徒","赌徒喽啰",wb[取随机数(1,#wb)],xx}
    		end
      	...
      	elseif 编号==19 then
    		...
    		wb[1] = "哈哈被你发现了"
    		xx = {"触发剧情"}
    		return {"强盗","江洋大盗",wb[取随机数(1,#wb)],xx}
    		end
        ...
        elseif 编号==20 then
    		...
    		wb[1] = "你是谁?找我何事"
    		xx = {"触发剧情"}
    		return {"骷髅怪","洛川鬼",wb[取随机数(1,#wb)],xx}
    		end
    		...
    		return {"骷髅怪","洛川鬼",wb[取随机数(1,#wb)],xx}
      		...
      		return {"骷髅怪","洛川鬼",wb[取随机数(1,#wb)],xx}
      elseif 编号 == 21 then
			wb[1] = "我是文韵墨香大唐国境的西域使者"
			return {"男人_马副将","大唐国境西域使者",wb[1],xx}
    ...
    elseif ID == 1070 then
	    ...
	    elseif 编号==17 then
	      	...
	      	xx = {"领养-小毛头","领养-小丫丫","领养-小仙灵","领养-小仙女","领养-小精灵","领养-小魔头","群里输入孩子名称查看技能","路过看看"}
	      	--xx = {"路过看看"}
    ...
    elseif ID == 1216 then--仙源洞天
  	...
  	elseif ID == 1040 then--西凉女国
	    ...
	    elseif 编号 == 6 then
       	...
       	elseif 编号 == 7 then
      		wb[1] = "哎哟不错,很吊哦!!"
      		--xx = {"我要取回角色","木桩伤害测试","我路过瞧瞧而已"}
      		xx = {"木桩伤害测试","商人的鬼魂(日常剧情)","妖风(日常剧情)","白鹿精(日常剧情)"}
       		return {"周杰伦","周吊伦",wb[取随机数(1,#wb)],xx}
       	elseif 编号 == 8 then
        ...
        elseif 编号 == 9 then
        	if 玩家数据[数字id].角色.数据.等级>=69 then
      		wb[1] = "想要突破69级等级上限,升级到109级,找我开启战斗吧！"
      		xx = {"开启突破战斗"}
       		return {"春十三娘","西凉国王",wb[取随机数(1,#wb)],xx}
       	   end
    	end
  	elseif ID == 1226 then
    	if 编号==1 then
  	...
  	elseif ID == 1019 then --书香斋
    	...
    	if 编号 == 1 then
    		 if 玩家数据[数字id].角色:取任务(19941)~=0 then --嘎嘎 文韵墨香
    		 	  任务处理类:设置文韵墨香题目(玩家数据[数字id].连接id,数字id)
        		玩家数据[数字id].文韵科举对话=true
        		return{"考官2","礼部侍郎官",format("#Y/第%s题：#W/%s",文韵墨香[数字id].答题.总数, 玩家数据[数字id].文韵科举数据.题目),玩家数据[数字id].文韵科举数据.答案}
      		end
  	...
  	elseif ID == 1875 then -- 聚义堂
	   ...
	   if 帮派缴纳情况[玩家数据[数字id].角色.数据.帮派数据.编号]==nil then
          ...
          帮派缴纳情况[玩家数据[数字id].角色.数据.帮派数据.编号].起始时间 = os.time()
	  	    ...
	  	    xx = {"领取本周帮费","帮费设置","帮派加成领取","提升规模","送我回长安","路过，随便看看"}
...
elseif 编号 == 17 then
	...
	elseif 编号 == 18 then
			wb[1] = "亲,爱你哟。"
			xx={"人物洗点","法术认证","取消法术认证","角色转换","装备转换","武器染色","补充法宝灵气"}
			return{"袁天罡","袁天罡",wb[取随机数(1,#wb)],xx}
   end
-------------------嘎嘎
  elseif ID == 5140 then--跨服地图
  	if 编号 == 1 then
  		 wb[1] = "闲来无事爱插花，有事没事就论道"
			xx={"人物伤害强化","人物气血强化","您插花去吧。"}
			return{"男人_胖和尚","嘎嘎人物",wb[取随机数(1,#wb)],xx}

    elseif 编号 == 2 then
    	wb[1] = "你有好吃的吗？我要吃好吃的"
			xx={"召唤兽伤害强化","小豆丁你继续吃"}
			return{"猪八戒","嘎嘎宠物",wb[取随机数(1,#wb)],xx}

		elseif 编号 == 3 then
    	wb[1] = "你瞅啥？"
			xx={"关于征战神州","关于乾坤十二界","告辞"}
			return{"男人_太上老君","二代监正",wb[取随机数(1,#wb)],xx}
