...
-- @Last Modified time: 2023-08-03 18:41:29
--======================================================================--
-- @作者: GGE研究群: 342119466
-- @创建时间:   2018-03-03 02:34:19
-- @Last Modified time: 2023-03-02 22:33:36
...
function 场景类_地图单位对话:地图单位对话(连接id,数字id,序列,标识,地图,fanse)
  local 类型=任务数据[标识].类型
  ...
  elseif 类型==170 then
    ...
    if 任务数据[标识].名称=="伤心的小猴" and 副本数据.齐天大圣.进行[副本id].进程 == 1 and 副本id == 任务数据[玩家数据[数字id].角色:取任务(191)].副本id and 副本数据.齐天大圣.进行[副本id].小猴老猴.伤心的小猴 ~= nil and 副本数据.齐天大圣.进行[副本id].小猴老猴.伤心的小猴>0 then
      ...
      玩家数据[数字id].角色:添加经验(100000,"伤心的小猴")
    ...
    elseif 任务数据[标识].名称=="临死的老猴" and 副本数据.齐天大圣.进行[副本id].进程 == 1 and 副本id == 任务数据[玩家数据[数字id].角色:取任务(191)].副本id and 副本数据.齐天大圣.进行[副本id].小猴老猴.临死的老猴 ~= nil and 副本数据.齐天大圣.进行[副本id].小猴老猴.临死的老猴 > 0 then
      ...
      玩家数据[数字id].角色:添加经验(100000,"临死的老猴")
  ...
  elseif 类型==318 then
      if 任务数据[标识].战斗==nil then
        ...
        对话数据.对话="最最最最最好的老G赠送给新手玩家们的福利，是否需要领取呢？"
  ...
  elseif 类型==8817 then
    if 剧情数据.渡劫.进行[数字id].进程==12 or 剧情数据.渡劫.进行[数字id].进程==13 then
...
---嘎嘎跨服
  elseif 类型==1994 then
    if 任务数据[标识].战斗==nil then
      对话数据.对话="如果你的实力已经达到登峰造极的地步，可以尝试挑战我。我乃跨服#R"..任务数据[标识].等级.."级#W，等级大于或小于我20以上的玩家无法对我发起挑战。你是否已经准备好对我发起挑战？"
      对话数据.选项={"你装尼玛呢","我只是路过"}
    else
      对话数据.对话="我正在战斗中，请勿打扰。"
    end
