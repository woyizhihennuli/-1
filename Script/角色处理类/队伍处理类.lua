...
-- @Last Modified time: 2023-02-27 21:19:10
...
function 队伍处理类:退出队伍(id,重组,助战)
  ...
  if 玩家数据[id].队长 then
    ...
    for n=1,#队伍数据[队伍id].成员数据 do
      if 玩家数据[队伍数据[队伍id].成员数据[n]]~=nil then
        玩家数据[队伍数据[队伍id].成员数据[n]].队伍=0
        if 玩家数据[队伍数据[队伍id].成员数据[n]].角色.多角色操作 then
           玩家数据[队伍数据[队伍id].成员数据[n]].角色.多角色操作= false
          发送数据(玩家数据[队伍数据[队伍id].成员数据[n]].连接id,159)
        end
        if 玩家数据[队伍数据[队伍id].成员数据[n]].连接id == nil then
           if 玩家数据[队伍数据[队伍id].成员数据[n]].子角色操作~=nil then
...
function 队伍处理类:同意入队(id,内容,重组)
  ...
  if 玩家数据[id].角色.数据.地图数据.编号==6010 and 玩家数据[id].角色.数据.帮派数据.编号~=玩家数据[对方id].角色.数据.帮派数据.编号 then
...
return
  ...
  if id==创建id then -- 自己创建队伍
    ...
    if 玩家数据[创建id].队伍==0 then
      ...
      if 地图==6003 or 地图==6004 or 地图==6005 or 地图==6006 then
        if 比武大会.玩家表[id].阵营~=比武大会.玩家表[创建id].阵营 then
          常规提示(id,"您只能和已方阵营的玩家组队")
          return 0
        end
      end
