...
-- @Last Modified time: 2024-01-22 17:11:56
--======================================================================--
...
function 坐骑资料库:获取坐骑1(id,模型,名称,类型,属性,等级,染色方案,技能组,资质组,成长,参战等级,属性表)
	local n = 读取坐骑()
	local mx = 模型 == "小毛头" or 模型 == "小丫丫"
	if mx then
		类型 = "孩子"
	end
	n:置新对象1(模型,名称,类型,属性,等级,染色方案,技能组,资质组,成长,参战等级,属性表,id)
	if #玩家数据[id].角色.数据.坐骑列表 <= 6 then
		insert(玩家数据[id].角色.数据.坐骑列表, n)
	else
		常规提示(id,玩家数据[id].角色.数据.名称.."携带的坐骑已到上限无法再获得坐骑，请放生一只坐骑")
...
function 坐骑资料库:取坐骑库(人物)
	...
	if 取随机数(1,2)==1 and (人物.模型 == "龙太子" or 人物.模型 == "骨精灵") then
		zqsQQ = "宝贝葫芦"
	end
	return zqsQQ
end
...
function 坐骑资料库:取坐骑(bb)
	...
	if bb == "宝贝葫芦" or bb == "蓝色狐狸" or bb == "黄金狮子" or bb == "七彩祥云" or bb == "粉红火鸡" or bb == "彩虹毛驴"
		or bb == "粉嫩小猪" or bb == "水晶芭蕉" or bb == "深海狂鲨" or bb == "雷霆黑豹" or bb == "九尾妖狐" or bb == "砖石小马" then
	    bbs[1] = 45
		bbs[2] = 200
		bbs[3] = 200
		bbs[4] = 200
