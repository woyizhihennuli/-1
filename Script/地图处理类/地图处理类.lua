...
-- @Last Modified time: 2023-08-09 13:19:49
--======================================================================--
--======================================================================--
local 地图处理类 = class()
...
function 地图处理类:初始化()
  ...
  self.地图编号={1994,1001,1002,1003,1004,1005,1006,1007,1008,1009,1012,1013,1014,1015,1016,1017,1018,1019,1020,1021,1022,1023,
  1024,1025,1026,1028,1029,1030,1031,1032,1033,1034,1035,1036,1037,1038,1040,1041,1042,1043,1044,1046,1049,1050,1051,1052,1054,
1056,1057,1070,1072,1075,1077,1078,1079,1080,1081,1082,1083,1085,1087,1090,1091,1092,1093,1094,1095,1098,1099,1100,1101,1103,1104,
1105,1106,1107,1110,1111,1112,1113,1114,1115,1116,1117,1118,1119,1120,1121,1122,1123,1124,1125,1126,1127,1128,1129,1130,1131,1132,
1133,1134,1135,1137,1138,1139,1140,1141,1142,1143,1144,1145,1146,1147,1149,1150,1152,1153,1154,1155,1156,1165,1167,1168,1170,1171,
1173,1174,1175,1177,1178,1179,1180,1181,1182,1183,1186,1187,1188,1189,1190,1191,1192,1193,1197,1198,1201,1202,1203,1204,1205,1206,
...
1233,1234,1235,1236,1237,1238,1239,1241,1242,1243,1245,1246,1248,1249,1250,1251,1252,1253,1256,1257,1258,1259,1272,1273,1306,1310,
...
1337,1338,1339,1340,1341,1342,1343,1380,1382,1400,1401,1402,1403,1404,1405,1406,1407,1408,1409,1410,1411,1412,1413,1414,1415,1416,
1417,1418,1420,1421,1422,1424,1425,1426,1427,1428,1429,1430,1446,1447,1501,1502,1503,1504,1505,1506,1507,1508,1509,1511,1512,1513,
1514,1523,1524,1525,1526,1527,1528,1529,1531,1532,1533,1534,1535,1536,1537,1605,1606,1607,1608,1810,1811,1812,1813,1814,1815,1820,
1821,1822,1823,1824,1825,1830,1831,1832,1833,1834,1835,1840,1841,1842,1843,1844,1845,1850,1851,1852,1853,1854,1855,1860,1861,1862,
1863,1864,1865,1870,1871,1872,1873,1874,1875,1876,1885,1886,1887,1888,1890,1891,1892,1910,1911,1912,1913,1914,1915,1916,1920,1930,
1931,1932,1933,1934,1935,1936,1937,1938,1939,1940,1941,1942,1943,1944,1945,1946,1947,1948,1949,1950,1951,1952,1953,1954,1955,1958,
1959,1960,1961,1962,1963,1964,1965,1966,1967,1968,1969,1970,1971,2000,4001,5000}
...
function 地图处理类:数据处理(id,序号,数字id,内容)
  ...
  elseif 序号==1002.1 then
    if  数字id~=nil and 玩家数据[数字id]~=nil and 玩家数据[数字id].角色.数据.地图数据.x ~= nil and 玩家数据[数字id].角色.数据.地图数据.y ~=nil then
  ...
  elseif 序号==1003 then --
    local 目标=单机版切换场景(内容.说明,玩家数据[数字id].角色.数据.地图数据.编号)
  ...
  elseif 序号==1005 then
    if 内容.注释 == "庭院" or 内容.注释 == "豪华庭院" then
      for i=1,#房屋数据 do
        if 房屋数据[i].ID == 数字id then
          房屋数据[i].庭院装饰[内容.编号].x = 内容.x
          房屋数据[i].庭院装饰[内容.编号].y = 内容.y
          for n, v in pairs(self.地图玩家[玩家数据[数字id].角色.数据.地图数据.编号]) do--向地图内玩家发送新玩家数据
...
function 地图处理类:移动坐标刷新(id,数字id,内容)
  ...
  if 玩家数据[数字id].队伍~=0 and 玩家数据[数字id].队长 and 玩家数据[数字id].移动数据.移动目标 ~= nil then
    ...
    for n=2,#队伍数据[玩家数据[数字id].队伍].成员数据 do
      ...
      if 取两点距离(角色xy,对方xy)>=self.队伍距离*n+5000 then --超出五倍距离重置坐标
  ...
  -----文韵墨香巡逻
  if 玩家数据[数字id].角色:取任务(19945)~=0 and 玩家数据[数字id].角色.数据.地图数据.编号==1026 and  取随机数()<=5 and (玩家数据[数字id].战斗==nil or 玩家数据[数字id].战斗==0) then
    战斗准备类:创建战斗(数字id,199409,玩家数据[数字id].角色:取任务(19945))
  end
  if 玩家数据[数字id].角色:取任务(19946)~=0  and  取随机数()<=5 and (玩家数据[数字id].战斗==nil or 玩家数据[数字id].战斗==0) then
    战斗准备类:创建战斗(数字id,199410,玩家数据[数字id].角色:取任务(19946))
  end
...
function 地图处理类:更新飞行状态(id)  --飞行
  local 地图编号=玩家数据[id].角色.数据.地图数据.编号
  for n, v in pairs(self.地图玩家[地图编号]) do
    if n~=id and self:取同一地图(地图编号,id,n,1) then
      发送数据(玩家数据[n].连接id,1010.1,{id=id,飞行=玩家数据[id].角色.数据.飞行})
    end
  end
end
...
function 地图处理类:取同一地图(地图,id,id1,类型)
  ...
  if (地图>=6000 and 地图<=6050) or (地图>=7000 and 地图<=7100) then
    if 类型==1 then --玩家
  ...
  elseif 地图==1210 or 地图==1092 or 地图==1179 or 地图==1187 or 地图==1178 or 地图==1210 or 地图==1208 or 地图==1210 or 地图==1042 or 地图==1206 then
    if 类型==1 then
      if 玩家数据[id] and 玩家数据[id1] and 玩家数据[id].分区 == 玩家数据[id1].分区 then
         return true
      end
      --return true
    else
    ...
    --   if 玩家数据[id] and 玩家数据[id1] and 玩家数据[id].分区 == 玩家数据[id1].分区 then
    --     return true
    --   elseif 地图 >= 5140 and 地图 <= 5157 then
    --     return true
    --   end
    --   return false
    -- else
    --   if 玩家数据[id] and 任务数据[id1.id] and 玩家数据[id].分区 == 任务数据[id1.id].分区 then
    --     return true
    --   elseif 地图 >= 5140 and 地图 <= 5157 then
    --     return true
    --   else
...
function 地图处理类:加入玩家(id,地图编号,x,y)
  ...
  发送数据(玩家数据[id].连接id,1003,取假人表(玩家数据[id].角色.数据.地图数据.编号))-- 你这个版本感觉都没改完啊.纯粹客户端读取NPC.
