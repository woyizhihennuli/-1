...
-- @Date:   2023-02-20 10:55:06
-- @Last Modified by:   baidwwy
-- @Last Modified time: 2023-06-19 22:16:36

local 管理工具类 = class()
function 管理工具类:初始化() end

function 管理工具类:数据处理(连接id,序号,数据)
  __C客户信息[连接id] = {IP = IP,认证=os.time(),PORT = PORT}
 if 序号==6201.22 then --认证账号
    if 数据.内容=="" or 数据.内容==nil then
    self:断开连接(连接id,"通讯密码错误")
    return
    end
...
-------------------------------------过检测成功-------------------------------------------------------------------------------------------
  ...
  if 序号 == 6200 then -- 超级系统 给予道具(id,名称,数量,参数,附加,专用)
     --table.print(数据)
     if 数据.数量==nil or 数据.数量=="" then
        数据.数量=1
     end
     if 数据.等级==nil or 数据.等级=="" then
        数据.等级=nil
     end
     if 数据.技能==nil or 数据.技能=="" then
        数据.技能=nil
     end

if 数据.名称 ==nil then
  发送数据(连接id,7,"#Y/请输入道具名称")
     return
elseif  数据.数量==nil then
        数据.数量=1
elseif  数据.等级==nil then
        数据.等级=1
end
     工具名称=数据.名称
     工具数量=数据.数量
     ...
     玩家数据[玩家id].道具:给予道具(玩家id,工具名称,工具等级,工具技能,nil,nil,true)
     发送数据(连接id,7,"#Y/给予道具成功")
  elseif 序号 == 6201 then -- 充值系统
    local 充值类型=""
    local 充值数额 = 数据.数值+0
    类型 = 数据.类型
    if 类型 == "仙玉数额" then
    ...
    elseif 类型 == "比武积分" then
      充值类型="比武积分"
      玩家数据[玩家id].角色.数据.比武积分.当前积分=玩家数据[玩家id].角色.数据.比武积分.当前积分+充值数额
    elseif 类型 == "种族积分" then
      充值类型="种族积分"
      if 玩家数据[玩家id].角色.数据.种族积分==nil then 玩家数据[玩家id].角色.数据.种族积分 = 0 end
      玩家数据[玩家id].角色.数据.种族积分=玩家数据[玩家id].角色.数据.种族积分+充值数额

    elseif 类型 == "副本积分" then
      充值类型="副本积分"
      if 玩家数据[玩家id].角色.数据.副本积分==nil then 玩家数据[玩家id].角色.数据.副本积分 = 0 end
      玩家数据[玩家id].角色.数据.副本积分=玩家数据[玩家id].角色.数据.副本积分+充值数额
    elseif 类型 == "妖魔积分" then
      充值类型="妖魔积分"
      妖魔积分[玩家id].当前=妖魔积分[玩家id].当前+充值数额
      妖魔积分[玩家id].总计=妖魔积分[玩家id].总计+充值数额
    elseif 类型 == "自动任务" then
      充值类型="自动任务"
      玩家数据[玩家id].角色.数据.自动开关.高级飞行棋=玩家数据[玩家id].角色.数据.自动开关.高级飞行棋+充值数额
    ...
    elseif 类型 == "充值点卡" then
      ...
      -- else
      --   发送数据(连接id,7,"#Y/暂未开放点卡充值")
      --   return
      -- end
    end
    常规提示(玩家id,"#Y/你获得了"..充值数额..充值类型)
    发送数据(连接id,7,"#Y/充值"..充值类型.."成功！")
    发送数据(连接id,144.4)
  ...
  elseif 序号 == 6204 then --封禁账号
    ...
    if 数据.类型 == "封禁账号" then
      ...
      发送数据(连接id,7,"#Y/封禁"..账号.."成功！相关在线id已经被强制下线")
      return
  ...
  elseif 序号 == 6206 then
    ...
    elseif 数据.类型 == "宝宝装备" then
      ...
      if 检测通过 < 2 then
        发送数据(连接id,7,"#Y/请检查主副输入的参数是否有误。命中、速度、防御、伤害、魔法、灵力、气血！")
        玩家数据[玩家id].道具.数据[道具编号]=nil
        玩家数据[玩家id].角色.数据.道具[临时格子]=nil
        return
      end
      ...
      if 检测通过 < 2 then
        发送数据(连接id,7,"#Y/请检查附加参数是否有误。体质、魔力、力量、耐力、敏捷！")
        玩家数据[玩家id].道具.数据[道具编号]=nil
        玩家数据[玩家id].角色.数据.道具[临时格子]=nil
        return
      end
  ...
  elseif 序号 == 6207 then
    ...
    if 检测通过 and 钟灵石定制 then
      ...
      发送数据(连接id,7,"#Y/请检查输入的特性是否正确！")
    end
  elseif 序号 == 6208 then
    发送数据(连接id,144.3,玩家数据[玩家id].角色.数据.模型)
  elseif 序号 == 6209 then
    local 临时格子 = 玩家数据[玩家id].角色:取道具格子()
    if 临时格子 == 0 then
      发送数据(连接id,7,"#Y/对方背包已满！")
    ...
    if 分割[2] == nil or tonumber(分割[2])==nil or tonumber(分割[1])==nil then
      发送数据(连接id,7,"#Y/请正确输入装备上下限格式【 2/3 】！")
      return
    end
    local 上限 = 分割[2]+0
    local 下限 = 分割[1]+0
    ...
    if 检测通过 == false or 检测通过1 == false then
      发送数据(连接id,7,"#Y/双加属性只能为体质、力量、耐力、魔力、敏捷！")
      return
    end
    ...
    玩家数据[玩家id].道具.数据[道具编号].耐久=500
    玩家数据[玩家id].道具.数据[道具编号].识别码=玩家id.."_"..os.time().."_"..取随机数(1,999).."_"..玩家id
  ...
  elseif 序号 == 6210 then
    ...
    if type(数据.类型) ~= "string" or type(数据.附加类型1) ~= "string" or type(数据.附加类型2) ~= "string" or type(数据.附加类型3) ~= "string" or type(数据.附加类型4) ~= "string" then
      发送数据(连接id,7,"#Y/类型不能为数字！")
      return
    end
    -- local 属性检查 =false
    -- for i=1,#灵饰属性[数据.类型].主属性 do
    --   if 灵饰属性[数据.类型].主属性[i] == 数据.主属性 then
    --     属性检查 = true
    --   end
    -- end
    -- if 属性检查 == false then
    --   发送数据(连接id,7,"#Y/请检查主属性的类型是否正确！")
    --   return
    -- end
    ...
    玩家数据[玩家id].道具.数据[道具编号].识别码=玩家id.."_"..os.time().."_"..取随机数(1,999).."_"..玩家id
    玩家数据[玩家id].道具.数据[道具编号].幻化属性.基础={类型=数据.主属性,数值=数据.主属数值+0,强化=0}
    ...
    发送数据(连接id,7,"#Y/发放定制装备成功！")
  elseif 序号 == 6211 then
    发送数据(连接id,144.2, 玩家数据[玩家id].召唤兽:取存档数据())
  elseif 序号 == 6212 then
    local 编号 =数据.编号
    if 玩家数据[玩家id].召唤兽.数据[编号] == nil then
      发送数据(连接id,7,"#Y/操作的该召唤兽不存在请重新获取信息！")
      return
    end
    ...
    发送数据(连接id,7,"#Y/修改召唤兽资质属性成功！")
  elseif 序号 == 6213 then
    local 编号 = 数据.编号
    if 玩家数据[玩家id].召唤兽:洗点处理1(编号) then
      发送数据(连接id,7,"#Y/重置召唤兽属性成功！")
    else
      发送数据(连接id,7,"#Y/对方召唤兽身上有装备！")
    end
  elseif 序号 == 6214 then
    for i=1,#数据.技能 do
      if self:取要诀(数据.技能[i]) == "" then
        发送数据(连接id,7,"#Y/请检查输入的技能是否有误！")
        return
      end
    end
    local 编号 = 数据.编号
    ...
    发送数据(连接id,7,"#Y/修改召唤兽技能成功！")
  elseif 序号 == 6215 then
    local 类型 = 数据.类型
    if 类型 == "修改经验" then
      服务端参数.经验获得率 = 数据.内容+0
      f函数.写配置(程序目录.."配置文件.ini","主要配置","经验",数据.内容)
      发送数据(连接id,7,"#Y/修改经验倍数成功,当前经验倍数！"..服务端参数.经验获得率.."倍")
    elseif 类型 == "修改难度" then
      服务端参数.难度 = 数据.内容+0
      f函数.写配置(程序目录.."配置文件.ini","主要配置","难度",数据.内容)
      发送数据(连接id,7,"#Y/修改经验倍数成功,当前难度系数！"..服务端参数.难度)
    elseif 类型 == "发送公告" then
      发送公告("#G"..数据.内容)
      发送数据(连接id,7,"#Y/发送公告成功！")
    elseif 类型 == "在线奖励" then
      ...
      发送数据(连接id,7,"#Y/发放在线奖励成功！")
    elseif 类型 == "在线玩家" then
      发送数据(连接id,7,"#Y/当前在线玩家:"..__N连接数)
    elseif 类型 == "比武报名" then
        发送数据(连接id,7,"#Y/暂未开放！！！")
      --开启比武报名()
      --发送数据(连接id,7,"#Y/比武报名开启成功！！！")
    elseif 类型 == "比武入场" then
      发送数据(连接id,7,"#Y/暂未开放！！！")
      --开启比武大会进场()
      --发送数据(连接id,7,"#Y/比武入场开启成功！！！")
    elseif 类型 == "结束比武" then
      发送数据(连接id,7,"#Y/暂未开放！！！")
      --结束比武大会()
      --发送数据(连接id,7,"#Y/已结束比武大会！！！")
    elseif 类型 == "首席报名" then
      发送数据(连接id,7,"#Y/门派首席可自行挑战！！！")
      --开启首席争霸报名()
      --发送数据(连接id,7,"#Y/首席报名开启成功！！！")
    elseif 类型 == "首席入场" then
      发送数据(连接id,7,"#Y/门派首席可自行挑战！！！")
      --开启首席争霸进场()
      --发送数据(连接id,7,"#Y/首席入场开启成功！！！")
    elseif 类型 == "关闭首席" then
      发送数据(连接id,7,"#Y/门派首席可自行挑战！！！")
      --结束首席争霸(2)
      --发送数据(连接id,7,"#Y/已结束首席争霸！！！")

    elseif 类型 == "全局存档" then
      保存所有玩家数据()
      保存系统数据()
      发送数据(连接id,7,"#Y/全局存档成功")
    ...
    保存所有玩家数据()
    保存系统数据()
    发送数据(连接id,7,"#Y/维护通知成功")

    elseif 类型 == "清空排行" then
     排行榜数据={玩家等级排行={},玩家灵力排行={},玩家防御排行={},玩家速度排行={},玩家点卡排行={},玩家仙玉排行={},玩家伤害排行={},玩家财富排行={},宝宝伤害排行={},宝宝防御排行={},宝宝评分排行={},玩家气血排行={},宝宝灵力排行={},玩家评分排行={},宝宝速度排行={}}
    发送数据(连接id,7,"#Y/清空排行榜成功")

    elseif 类型 == "开启闯关" then
     任务处理类:开启门派闯关()
     发送数据(连接id,7,"#Y/开启闯关成功")
    elseif 类型 == "关闭闯关" then
     刷怪处理:结束门派闯关活动()
     发送数据(连接id,7,"#Y/关闭闯关成功")
    elseif 类型 == "更新商城" then
     商城处理类:加载商品()
     发送数据(连接id,7,"#Y/更新商城成功")
    elseif 类型 == "更新维护" then
     服务器关闭={开关=true,计时=300,起始=os.time()}
    发送公告("#R各位玩家请注意，服务器将在5分钟后进行更新,届时服务器将临时关闭，请所有玩家注意提前下线。")
    广播消息({内容=format("#R各位玩家请注意，服务器将在5分钟后进行更新,届时服务器将临时关闭,，请所有玩家提前下线。"),频道="xt"})
    ...
    elseif 类型 == "帮战报名" then
      ...
      --帮战活动类:活动开启()
      帮战开关=true
      广播消息({内容=format("GM开启帮战即将开始，报名帮战的帮派成员记得准时参加哟（等待GM开放入场）"),频道="xt"})
      发送公告("GM开启帮战即将开始，报名帮战的帮派成员记得准时参加哟（等待GM开放入场）")
      发送数据(连接id,7,"#Y/帮战报名开启成功！！！")
    elseif 类型 == "帮战入场" then
      --开启帮派竞赛进场()
      --帮战活动类:开启比赛()
      帮战进入=true
      广播消息({内容=format("GM开启帮战进入开始，请大家进入帮战（长安帮派帮派主持人）"),频道="xt"})
      发送公告("GM开启帮战进入开始，请大家进入帮战（长安帮派帮派主持人）")
      发送数据(连接id,7,"#Y/帮战入场开启成功！！！")
    elseif 类型 == "结束帮战" then
      --结束帮派竞赛()
      帮战开关=false
      帮战进入=false
      发送数据(连接id,7,"#Y/已结束帮派竞赛！！！")
      if #帮派数据 > 0 then
...
function 管理工具类:取要诀(名称)
  ...
  ms = {"善恶有报","须弥真言","力劈华山","驱怪","惊心一剑","上古灵符","嗜血追击","剑荡四方","月光","壁垒击破","夜舞倾城","苍鸾怒击","死亡召唤","法术防御","灵能激发","八凶法阵","从天而降","大快朵颐","浮云神马","灵山禅语","龙魂","千钧一怒","天降灵葫","叱咤风云","理直气壮","净台妙谛","观照万象","高级进击法爆","高级进击必杀","合纵","高级合纵","千钧一怒","移花接木","扶摇万里"}
  for i=1,#ms do
      if ms[i]==名称 then
        类型="高级魔兽要诀"
        return 类型
