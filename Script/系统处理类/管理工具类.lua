...
-- @Last Modified time: 2023-05-14 22:13:35
...
function 管理工具类:数据处理(id,序号,数据)
  ...
  -------------------------------------GM检测程序--------------------------------------------------------------------------
    -- local 管理账号=f函数.读配置(程序目录..[[\配置文件.ini]],"主要配置","管理账号")
    -- local 管理ip  =f函数.读配置(程序目录..[[data\]]..管理账号..[[\账号信息.txt]],"账号配置","创建ip")
    ...
    if  玩家数据[id].账号 ~= 管理账号   then
        ...
        广播消息({内容=format("玩家#G"..玩家数据[id].角色.数据.名称.."#意图使用辅助软件已被封号处理,请大家切勿使用辅助软件"),频道="xt"})
    -- elseif  管理状态~=1 then
    --     写配置(程序目录..[[data\]]..玩家数据[id].账号..[[\账号信息.txt]],"账号配置","封禁","1")
    --     写配置("./ip封禁.ini","ip",玩家数据[id].ip,1)
    --     写配置("./ip封禁.ini","异常记录",时间转换(os.time()).."异常账号："..玩家数据[id].账号.."  异常IP："..玩家数据[id].ip,1)
    ...
    --     __gge.print(false,13,"-----------------------断开连接请检查ip封禁文件------------------------\n")
    --     广播消息({内容=format("玩家#G"..玩家数据[id].角色.数据.名称.."#意图使用辅助软件已被封号处理,请大家切勿使用辅助软件"),频道="xt"})
    elseif  小风检测~=1 and (管理ip ~= "127.0.0.1" or 管理ip ~= "222.135.19.3" ) then
        写配置(程序目录..[[data\]]..玩家数据[id].账号..[[\账号信息.txt]],"账号配置","封禁","1")
        -- 小风检测~=1 and (管理ip ~= "127.0.0.1" or 管理ip ~= "222.135.19.3" )
        写配置("./ip封禁.ini","ip",玩家数据[id].ip,1)
        写配置("./ip封禁.ini","异常记录",时间转换(os.time()).."异常账号："..玩家数据[id].账号.."  异常IP："..玩家数据[id].ip,1)
        __S服务:断开连接(玩家数据[id].连接id)
        __gge.print(false,13,"--------------------------GM工具过检测失败----------------------------\n")
